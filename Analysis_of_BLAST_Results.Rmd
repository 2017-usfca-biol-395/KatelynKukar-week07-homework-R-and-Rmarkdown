...
joined_blast_data_metadata %>%
  ...
  pull(pident) %>%
  hist(main = "Percent Identity of Human Environmental Material within Whole Data",
       xlab = "Percent")
...
joined_blast_data_metadata %>%
  ...
  filter(sex_s == "female") %>%
  pull(pident) %>%
  hist(main = "Percent of Human Environmental Material Analyzed for Females",
       xlab = "Percent")

# Histogram to visualize sex of human hosts which were found in relationship to comp. mice
joined_blast_data_metadata %>%
  filter(env_material_s == "sebum") %>%
  filter(sex_s == "female") %>%
  filter(sample_type_s == "computer mouse") %>%
  pull(pident) %>%
  hist(main = "Percent of Human Environmental Material Analyzed for Females on Computer Mice",
       xlab = "Percent")
...
joined_blast_data_metadata %>%
  ...
  filter(sex_s == "female") %>%
  pull(length) %>%
  hist(main = "Query Sequence Length of Human Environmental Material Analyzed for Females",
       xlab = "Length in BP")


# Histogram to visualize length of sequences based on human host and sex which were found in relationship to comp. mice
joined_blast_data_metadata %>%
  filter(env_material_s == "sebum") %>%
  filter(sex_s == "female") %>%
  filter(sample_type_s == "computer mouse") %>%
  pull(length) %>%
  hist(main = "Query Sequence Length of Human Environmental Material Analyzed for Females on Computer Mice",
       xlab = "Length in BP")
