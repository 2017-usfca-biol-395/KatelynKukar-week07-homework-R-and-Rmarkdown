...
joined_blast_data_metadata %>%
  ...
  pull(pident) %>%
  hist(main = "Figure 1a. Percent Identity of Human Environmental Material within Whole Data",
       xlab = "Percent")
```
...
joined_blast_data_metadata %>%
  ...
  filter(sex_s == "female") %>%
  pull(pident) %>%
  hist(main = "Figure 1b. Percent of Human Environmental Material Analyzed for Females",
       xlab = "Percent")
```
...
joined_blast_data_metadata %>%
  filter(env_material_s == "sebum") %>%
  pull(length) %>%
  hist(main = "Figure 2a. Query Sequence Length of Human Environmental Material within Whole Data",
       xlab = "Length in BP")
```
...
joined_blast_data_metadata %>%
  ...
  filter(sex_s == "female") %>%
  pull(length) %>%
  hist(main = " Figure 2a. Query Sequence Length of Human Environmental Material Analyzed for Females",
       xlab = "Length in BP")
```
...
group_by(sscinames) %>%
  ...
  arrange(desc(n)) %>%
  head (5) %>%
  ggplot(aes(main = "Table 2a. Species visualized for Human Environmnetal Material Analyzed for Humans", 
             x = sscinames,
             y = n)) +
...
joined_blast_data_metadata %>%
  ...
  pull(pident) %>%
  hist(main = "Figure 3a. Percent Identity of Human Environmental Material Analyzed for Females for Most Common Community Found",
       xlab = "Percent")
```
...
joined_blast_data_metadata %>%
  ...
  filter(sscinames == "Bartonella washoensis") %>%
  pull(length) %>%
  hist(main = "Figure 3b. Query Sequence Length of Human Environmental Material Analyzed for Females for Most Common Community Found",
       xlab = "Length in BP")
```
...
joined_blast_data_metadata %>%
  ...
  pull(pident) %>%
  hist(main = "Figure 4a. Percent Identity of Human Environmental Material Analyzed for Males for Most Common Community Found",
       xlab = "Percent")
```
...
joined_blast_data_metadata %>%
  ...
  filter(sscinames == "Solemya pervernicosa gill symbiont") %>%
  pull(length) %>%
  hist(main = "Figure 4b. Query Sequence Length of Human Environmental Material Analyzed for Males for Most Common Community Found",
       xlab = "Length in BP")
```
...
# ggplot to look at lengths based on female and males and where the species came from
ggplot(joined_blast_data_metadata,
       aes(x = length,
           fill = sample_type_s)) +
